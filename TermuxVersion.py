...
def download_youtube(query, output_path, is_video=False):
    ...
    command = ["yt-dlp", "-o", output_template, "--quiet", "--no-warnings"]
        ...
        with Progress(
            ...
            for i, item in enumerate(tracks, 1):
                ...
                if os.path.exists(filename):
                    log(f"[{i}] Skipping (exists): {query}", "warning")
                    progress.advance(task)
                    continue

                success = download_youtube(query, full_path)
                if success:
                    log(f"[{i}] Downloaded: {query}", "success")
                else:
                    log(f"[{i}] Failed: {query}", "error")
...
def menu():
    config = load_config()
    while True:
        console.print("\n[bold magenta]--- Revolutionary Nonsense ---[/]\n")
        console.print("[purple]1.[/] Convert Spotify Playlist")
        console.print("[purple]2.[/] Download Single YouTube Video or Music")
        console.print("[purple]3.[/] Configure Spotify API")
        console.print("[purple]4.[/] Set Output Directory")
        console.print("[purple]5.[/] Exit")
...
if __name__ == "__main__":
    try:
        ...
        console.print("[red][x] yt-dlp is not installed.[/]")
        console.print("    Run: [bold]pkg install yt-dlp -y[/]")
        sys.exit(1)

    try:
        import spotipy
    except ImportError:
        console.print("[red][x] spotipy is not installed.[/]")
        console.print("    Run: [bold]pip install spotipy[/]")
        sys.exit(1)

    os.system("clear")
    menu()        return spotipy.Spotify(auth_manager=auth)
    except Exception as e:
        log(f"Spotify client error: {e}", "error")
        return None
